...
public class ExtensionContent {
    ...
    public File getGloabSniff() throws IOException {
        //查找指定资源的URL，其中res.txt仍然开始的bin目录下
        return ResourceUtils.getFile("classpath:/resource/extensions");
    }
    ...
    public static void load() throws IOException {
        ...
        ClassPathResource resource = new ClassPathResource("/extensions/gloabSniff/manifest.json");
        if (resource.exists()) {
            ExtensionInfo extensionInfo = new ObjectMapper().readValue(resource.getInputStream(), ExtensionInfo.class);
            extensionInfo.setMeta(Meta.load("classpath:extensions/gloabSniff", extensionInfo.getTitle()));
        ...
        if (file.exists() && file.isDirectory()) {
            ...
            for (File extendDir : file.listFiles()) {
                if (extendDir.isDirectory()) {
                    ...
                    if (extensionInfo != null && !EXTENSION_INFO_LIST.contains(extensionInfo)) {
                        EXTENSION_INFO_LIST.add(extensionInfo);
                    }
                }
            }
    ...
    private static ExtensionInfo parseExtensionDir(File extendDir) {
        ...
        if (extensionInfo != null) {
            Meta meta = Meta.load(extendDir.getPath(), extensionInfo.getTitle());
