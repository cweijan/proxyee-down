...
/**
 * 重写了{@link HttpDownUtil#getDownFileName(HttpRequest, HttpHeaders)}方法,对重复文件名自动生成序列号
 */
public class HttpDownUtil {
    ...
    public static String getDownFileName(HttpRequest httpRequest, HttpHeaders resHeaders) {
        ...
        return createUniqueFileName(fileName, 0);
    }

    private static String createUniqueFileName(String fileName, Integer deep) {
        if (deep > 100) {
            return fileName;
        }
        ...
        if (serverConfigInfo != null) {
            filePath = serverConfigInfo.getFilePath();
        }
        if (StringUtils.isEmpty(filePath) || !new File(filePath + File.separator + fileName).exists()) return fileName;

        String preSerialNumber = "(" + (deep) + ")";
        String serialNumber = "(" + (++deep) + ")";
        if (!fileName.contains(".")) {
            fileName += serialNumber;
        } else {
            if (deep > 1) {
                fileName = fileName.replace(preSerialNumber, serialNumber);
            } else {
                fileName = fileName.replace(".", serialNumber + ".");
            }
        }

        return createUniqueFileName(fileName, deep);
    }
